...
glm::vec3 objectPosition(0.0f); // Position of the object
glm::vec3 objectScale(1.0f);   
glm::vec3 objectRotation(0.0f);  // Rotation in degrees
glm::mat4 scaling = glm::mat4(1.0f); // Initialize scaling matrix
const float ROTATION_SPEED = 50.0f; // Velocidad de rotación en grados por segundo
const float TRANSLATION_SPEED = 5.0f; // Velocidad de movimiento
//-----------------------------------------------------------------------------
...
int main()
{
	...
	while (!glfwWindowShouldClose(gWindow))
	{
		...
		// Render the scene
		glm::mat4 position = glm::translate(glm::mat4(1.0), objectPosition);
		glm::mat4 rotationX = glm::rotate(glm::mat4(1.0), glm::radians(objectRotation.x), glm::vec3(1.0f, 0.0f, 0.0f));
		glm::mat4 rotationY = glm::rotate(glm::mat4(1.0), glm::radians(objectRotation.y), glm::vec3(0.0f, 1.0f, 0.0f));
		glm::mat4 rotationZ = glm::rotate(glm::mat4(1.0), glm::radians(objectRotation.z), glm::vec3(0.0f, 0.0f, 1.0f));

		model = position * rotationY * rotationX * rotationZ * scaling;
...
//-----------------------------------------------------------------------------
void update(double elapsedTime) {
    // Movimiento en el eje X (subir/bajar con W/S)
    if (glfwGetKey(gWindow, GLFW_KEY_W) == GLFW_PRESS)
        objectRotation.x += ROTATION_SPEED * (float)elapsedTime; // Rotar en Y positivo
    if (glfwGetKey(gWindow, GLFW_KEY_S) == GLFW_PRESS)
        objectRotation.x -= ROTATION_SPEED * (float)elapsedTime; // Rotar en Y positivo

    // Rotación en el eje Y (Q/E)
    if (glfwGetKey(gWindow, GLFW_KEY_Q) == GLFW_PRESS)
        objectRotation.y += ROTATION_SPEED * (float)elapsedTime; // Rotar en Y positivo
    if (glfwGetKey(gWindow, GLFW_KEY_E) == GLFW_PRESS)
        objectRotation.y -= ROTATION_SPEED * (float)elapsedTime; // Rotar en Y positivo

    // Acercar/Alejar en el eje Z (A/D)
    if (glfwGetKey(gWindow, GLFW_KEY_A) == GLFW_PRESS)
        objectPosition.z -= TRANSLATION_SPEED * (float)elapsedTime; // Acerca
    if (glfwGetKey(gWindow, GLFW_KEY_D) == GLFW_PRESS)
        objectPosition.z += TRANSLATION_SPEED * (float)elapsedTime; // Aleja
    ...
    {
        // Move object up
        objectPosition.y += TRANSLATION_SPEED * elapsedTime;
    }
    ...
    // Movimiento en el eje X (Flechas LEFT/RIGHT)
    if (glfwGetKey(gWindow, GLFW_KEY_LEFT) == GLFW_PRESS)
        objectPosition.x -= TRANSLATION_SPEED * (float)elapsedTime; // Izquierda
    if (glfwGetKey(gWindow, GLFW_KEY_RIGHT) == GLFW_PRESS)
        objectPosition.x += TRANSLATION_SPEED * (float)elapsedTime; // Derecha
}
//-----------------------------------------------------------------------------
